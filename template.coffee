...
exports.template = (grunt, init, done) ->
  ...
  init.process {}, [{
    ...
    default: 'Y/n',
    warning: 'Yes: QUnit unit tests + JSHint "browser" globals. ' +
      'No: Nodeunit unit tests.'
  }, {
    name: 'min_concat',
    ...
    default: 'Y/n',
    warning: 'This changes how filenames are determined and banners are '+
      'generated.'
  }], (err, props) ->
    props.dom = /y/i.test(props.dom)
    props.min_concat = /y/i.test(props.min_concat)
    props.package_json = /y/i.test(props.package_json)
    props.test_task = if props.dom then 'qunit' else 'nodeunit'
    props.file_name =
    ...
    prefer = (arr, preferred) ->
      for i in [0 ... preferred.length]
        if arr.indexOf(preferred[i]) != -1
          return preferred[i]
      preferred[0]

    # Guess at some directories, if they exist.
    dirs = grunt.file.expand({filter: 'isDirectory'}, '*').map (d) ->
      d.slice(0, -1)
    ...
    props.jquery = grunt.file
      .expand({filter: 'isFile'}, '**/jquery*.js').length > 0
